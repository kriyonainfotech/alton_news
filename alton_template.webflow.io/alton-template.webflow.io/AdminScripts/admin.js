...
//                 <a href="/news/${news.slug}" class="news-link-v3 w-inline-block">
...
//                             <button class="update-btn" data-id="${news._id}">Update</button>
//                         </div>
//                     </div>
//                 </a>
//             </div>
//         `).join(""); // Use join() to convert array into a single HTML string
...
//                     try {
//                         const res = await fetch(`http://localhost:3000/news/delete/${newsId}`, {
//                             method: "DELETE",
//                         });
...
//         // Add update functionality (redirect to update form)
//         document.querySelectorAll(".update-btn").forEach(button => {
//             button.addEventListener("click", function (event) {
//                 event.preventDefault();
//                 const newsId = this.getAttribute("data-id");
//                 window.location.href = `/alton_template.webflow.io/alton-template.webflow.io/updateNews.html?id=${newsId}`;
//             });
//         });
...
const deleteNewsItem = async (newsId) => {
    alert(newsId)
    try {
        const response = await fetch("http://localhost:3000/news/deleteNews", {
            method: "DELETE",
            headers: {
...
async function submitUpdate() {
    ...
    try {
        const response = await fetch("http://localhost:3000/news/updateNews", {
            method: "PUT",
            body: formData,
        });
